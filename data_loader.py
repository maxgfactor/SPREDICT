...
class DataManager:
    ...
    def _load_and_validate_csv(self, data_path: str, winsorize: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Drop rows where all non-Ticker_id features are exactly 0 (missing-data placeholders)
        ticker_idx = self._feature_columns.index('Ticker_id')
        feature_indices = [i for i in range(X.shape[1]) if i != ticker_idx]
        all_zero_mask = (X[:, feature_indices] == 0).all(axis=1)
        if all_zero_mask.any():
            count = all_zero_mask.sum()
            X = X[~all_zero_mask]
            y = y[~all_zero_mask]
            dates = dates[~all_zero_mask]
            if hasattr(self, 'logger') and self.logger:
                self.logger.log(f"   [data] Dropped {count} rows with all-zero features (missing data placeholder)", 'warning')
        
        # Store filtered y as raw_target_values (matches X exactly after NaN and zero-row removal)
        self._raw_target_values = y.copy()
