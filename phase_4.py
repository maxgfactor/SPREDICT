...
class ModelTraining(BasePhase):
    ...
    def execute(self, context: Dict) -> Dict:
        ...
        for arch_name in architectures:
            try:
                ...
                if all_thresholds_rejected:
                    self.logger.log(f"[section 1] {arch_tag} [label_threshold_optimal] [warning] All label thresholds rejected — using baseline metrics at LT=0.0 (fallback)", 'warning')
                ...
                if enable_hyperparam:
                    ...
                    hpo_result = self.hyperparam_optimizer.optimize(
                        arch_name=arch_name,
                        X_train=X_train_opt,
                    ...
                    if len(hpo_result) == 6:
                        best_hyperparams, hpo_best_model, hpo_best_precision, hpo_raw_model, hpo_raw_precision, hpo_raw_params = hpo_result
                    else:
                        best_hyperparams, hpo_best_model, hpo_best_precision = hpo_result
                        hpo_raw_model, hpo_raw_precision, hpo_raw_params = None, 0.0, {}
                    ...
                    if hpo_best_model is not None:
                        ...
                        # Also check raw-precision-best trial if different from balanced-score-best
                        if hpo_raw_model is not None and hpo_raw_model != hpo_best_model and not hpo_improved:
                            hpo_raw_val_pred = hpo_raw_model.predict(X_val_opt, verbose=0).flatten()
                            hpo_raw_val_binary = (hpo_raw_val_pred >= eval_pred_threshold).astype(int)
                            hpo_raw_val_precision = self.evaluator.calculate_precision(y_val_binarized, hpo_raw_val_binary)
                            if hpo_raw_val_precision > pre_hpo_val_precision:
                                hpo_improved = True
                                hpo_best_model = hpo_raw_model
                                hpo_val_pred = hpo_raw_val_pred
                                hpo_val_precision = hpo_raw_val_precision
                                best_hyperparams = hpo_raw_params
                                hpo_cm = self.evaluator.calculate_confusion_matrix(y_val_binarized, hpo_raw_val_binary)
                                hpo_TP = hpo_cm['TP']
                                hpo_FP = hpo_cm['FP']
                                hpo_TN = hpo_cm['TN']
                                hpo_FN = hpo_cm['FN']
                                hpo_post_precision = hpo_raw_val_precision
                                hpo_R = self.evaluator.calculate_recall(y_val_binarized, hpo_raw_val_binary)
                                hpo_AUC = self.evaluator.calculate_auc(y_val_binarized, hpo_raw_val_pred)
                                hpo_F1 = self.evaluator.calculate_f1(y_val_binarized, hpo_raw_val_binary)
                                # Update logging hyperparams to match promoted trial
                                hpo_loss = best_hyperparams.get('loss_function', 'binary_crossentropy')
                                hpo_lr = best_hyperparams.get('learning_rate', 0.001)
                                hpo_dropout = best_hyperparams.get('dropout', 0.1)
                                hpo_alpha = best_hyperparams.get('alpha', 0.0)
                                hpo_gamma = best_hyperparams.get('gamma', 0.0)
                                hpo_latent_dim = best_hyperparams.get('latent_dim', 0)
                                self.logger.log(f"   raw-precision hyperparameter_optimization improved: {hpo_raw_val_precision:.4f} > {pre_hpo_val_precision:.4f}", 'info')
                ...
                if use_post_hpo_model:
                    ...
                    self.logger.log(
                        f"   Retraining post-HPO model at LABEL_THRESHOLD={final_threshold:.1f} "
                        f"for {train_epochs} epochs (post_hpo_prec={post_hpo_prec:.4f} > "
                ...
                # Use the best model: HPO only if it actually improved at optimal_threshold
                    ...
                    train_epochs = self.config['FINAL_TRAIN_EPOCHS'].get(arch_name, 3)
                    self.logger.log(f"   Retraining HPO model for {train_epochs} epochs (FINAL_TRAIN_EPOCHS)", 'info')
                    model = self.model_trainer.build_architecture_with_params(
                        arch_name, X_train_opt.shape[1], best_hyperparams
                    )
                    if hasattr(model, 'sklearn_model'):
                ...
                arch_final_metrics.append({
                    ...
                    # NEW: HPO details (read from config — was hardcoded 20 before Jul 11, 2026)
                    'hpo_trials': self.config['HYPERPARAM_OPTIMIZATION_TRIALS'],
        ...
        if not trained_models:
            ...
            # Filter architectures by minimum validation_precision
            self.logger.log(f"Filtering architectures by minimum validation_precision > {ensemble_min_precision:.2f}:", 'info')
            for i, (arch_name, model) in enumerate(zip(arch_names, trained_models)):
                if model is not None and i < len(best_val_precision_list):
                    ...
                    if val_prec > ensemble_min_precision:
                        ...
                        self.logger.log(f"  {arch_name}: validation_precision={val_prec:.4f} PASS (Minimum Validation_Precision Required={ensemble_min_precision:.2f})", 'info')
                    else:
                        self.logger.log(f"  {arch_name}: validation_precision={val_prec:.4f} FAIL (Minimum Validation_Precision Required={ensemble_min_precision:.2f})", 'info')
            ...
            self.logger.log(f"Ensemble precision (validation): {ensemble_precision:.4f} (LABEL_THRESHOLD={opt_threshold:.1f}, prediction_binary_split={self.config['PREDICTION_THRESHOLD']:.2f})", 'info')
        ...
        if arch_final_metrics:
            ...
            save_models = self.config['SAVE_TRAINED_MODELS']
